...
export const createDeliveryOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Номер заказа получаем только при создании реального заказа, чтобы счётчик
    // не сбивался, если покупатель закрыл окно оплаты.
    const { data: codeData, error: codeError } = await (supabaseAdmin as any).rpc("next_order_code");
    const orderNumber = (!codeError && codeData?.[0]?.code) || `RS-${Date.now().toString().slice(-3)}`;
    const orderSeq = (!codeError && codeData?.[0]?.seq) || null;
    ...
    try {
      ...
      await sendTemplateEmail("order-notification", "", {
        ...
        templateData: {
          orderNumber,
          orderSeq: orderSeq ? String(orderSeq) : "—",
          orderId: order.id,
